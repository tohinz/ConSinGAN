...
def train(opt, Gs, Zs, reals, NoiseAmp):
    ...
    while scale_num<opt.stop_scale+1-opt.start_scale:
        ...
        if scale_num > 1:
            ...
            G_curr.init_next_block(scale_num)

        writer = SummaryWriter(log_dir=opt.outf)
        Zs, NoiseAmp, G_curr, D_curr = train_single_scale(D_curr, G_curr, reals, Zs, NoiseAmp, opt, scale_num, writer, netDs_ratio)
        generate_samples(G_curr, opt, scale_num, m_pad, m_pad_block, NoiseAmp, writer, reals=reals)
...
def train_single_scale(netD, netG, reals, Zs, NoiseAmp, opt, depth, writer):
    ...
    real = reals[depth]
    print("Working on image size:", real.shape)
    ...
    for iter in _iter:
        ...
        if iter == 0 and depth == 0:
            ...
            z_reconstruction = netG(Zs, depth-1, reals_shapes, m_pad, m_pad_block, NoiseAmp)
        ...
        for j in range(opt.Dsteps):
            ...
            # train with fake
            if j == opt.Dsteps - 1:
                fake = netG(noise, depth, reals_shapes, m_pad, m_pad_block, NoiseAmp)
            else:
                with torch.no_grad():
                    fake = netG(noise, depth, reals_shapes, m_pad, m_pad_block, NoiseAmp)
        ...
        if alpha != 0:
            loss = nn.MSELoss()
            rec = netG(Zs, depth, reals_shapes, m_pad, m_pad_block, NoiseAmp, real_image=reals[depth].detach(),
                    reconstruct=True)
        ...
        if iter % 500 == 0 or iter == (opt.niter - 1):
            save_image('{}/fake_sample_{}.png'.format(opt.outf, iter), fake.detach())
            save_image('{}/reconstruction_{}.png'.format(opt.outf, iter), rec.detach())
            generate_samples(netG, opt, scale, m_pad, m_pad_block, NoiseAmp, writer, reals_shapes)

        schedulerD.step()
        schedulerG.step()
        # break

    functions.save_networks(netG, netD, z_opt, opt, netDs_ratio)
    return Zs, NoiseAmp, netG, netD


def generate_samples(netG, opt, scale, m_pad, m_pad_block, NoiseAmp, writer, reals_shapes, n=5):
    def save_image(name, image, opt):
        plt.imsave(name, functions.convert_image_np(image, opt), vmin=0, vmax=1)

    opt.out_ = functions.generate_dir2save(opt)
    dir2save = '{}/gen_samples_stage_{}'.format(opt.out_, scale_num)
    try:
            ...
            for d in range(scale + 1):
                if d == 0:
                    ...
                    noise.append(functions.generate_noise([opt.nfc,
                                                           ...
                                                           reals_shapes[d][3] + opt.num_layer * 2],
                                                           device=opt.device).detach())
            sample = netG(noise, scale, reals_shapes, m_pad, m_pad_block, NoiseAmp)
            all_images.append(sample)
            save_image('{}/gen_sample_{}.png'.format(dir2save, idx), sample.detach, opt)
